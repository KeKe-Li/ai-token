...
func (s *LogStore) Create(ctx context.Context, log *RequestLog) error {
	_, err := s.db.Exec(ctx,
		...
		log.UserID, log.APIKeyID, log.ChannelID, log.Model, log.RequestMethod, log.RequestPath,
		log.StatusCode, log.InputTokens, log.OutputTokens, log.Cost, log.LatencyMs,
...
func (s *LogStore) ListByUser(ctx context.Context, userID int64, limit, offset int) ([]RequestLog, error) {
	rows, err := s.db.Query(ctx,
		`SELECT id, user_id, api_key_id, channel_id, model, request_method, request_path, status_code, input_tokens, output_tokens, cost, latency_ms, error_message, ip_address, created_at
	...
	for rows.Next() {
		var l RequestLog
		if err := rows.Scan(&l.ID, &l.UserID, &l.APIKeyID, &l.ChannelID, &l.Model,
			&l.RequestMethod, &l.RequestPath, &l.StatusCode, &l.InputTokens,
			&l.OutputTokens, &l.Cost, &l.LatencyMs, &l.ErrorMessage,
