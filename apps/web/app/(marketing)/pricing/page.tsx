...
import { FALLBACK_MODELS, PROVIDER_LABELS, type PricingModel } from "@/lib/fallback-models";

export default function PricingPage() {
  const [models, setModels] = useState<PricingModel[]>(FALLBACK_MODELS);

  useEffect(() => {
    api.get<{ data: PricingModel[] }>("/api/public/models")
            ...
            {models.map((m) => (
              <tr key={m.model_id} className="border-b border-border hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className="rounded-md bg-primary/10 px-2 py-0.5 text-xs text-primary">{PROVIDER_LABELS[m.provider] ?? m.provider}</span>
                </td>
