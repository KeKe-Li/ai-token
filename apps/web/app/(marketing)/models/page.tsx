...
const CATEGORIES = ["all", "llm", "embedding", "image", "audio"] as const;
...
function ModelCard({ model }: { model: Model }) {
  const colorClass = PROVIDER_COLORS[model.provider] || "bg-muted text-muted-foreground";
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <span className={`mt-1 inline-block rounded-md border px-2 py-0.5 text-xs ${colorClass}`}>
            {model.provider}
          </span>
        </div>
